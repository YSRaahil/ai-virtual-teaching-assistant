...
All API routes live here. Business logic lives in models/grading/analytics/auth.
Routes are organised by role: auth, student, teacher, admin, ai.
"""
...
if GEMINI_API_KEY:
    ...
    log.warning("⚠️  GEMINI_API_KEY not set — AI features disabled")
...
def err(message="An error occurred", code=400):
    return jsonify({"status": "error", "message": message}), code


def require_json(*fields):
    """Validate that required fields exist in request JSON. Returns (data, error_response)."""
...
def register():
    ...
    Role must be: student | teacher (admin created manually)
    """
    data, error = require_json("name", "email", "password", "role")
    if error:
        return error
...
def get_course(current_user, course_id):
    """Get a single course with its assignments."""
...
def enroll(current_user, course_id):
    ...
    return ok({"course_id": course_id}, "Enrolled successfully", 201)
...
def create_assignment(current_user, course_id):
    ...
    rubric_keywords: comma-separated keywords used for auto-grading.
    e.g. "photosynthesis, chlorophyll, sunlight, glucose, oxygen"
    """
    data, error = require_json("title", "rubric_keywords")
    if error:
        return error
...
def get_assignment(current_user, assignment_id):
    """Get assignment details. Students also see their own submission if any."""
...
def submit(current_user, assignment_id):
    """
    Student submits an answer. Triggers auto-grading immediately.
    Body: { content: "student's answer text" }
    """
...
@role_required("teacher", "admin")
def list_submissions(current_user, assignment_id):
    """Teacher views all submissions for an assignment with grades."""
    assignment = models.get_assignment_by_id(assignment_id)
    if not assignment:
        return err("Assignment not found", 404)
    ...
    # Class analytics
    class_stats = analytics.class_performance_summary(
        submissions, assignment["max_score"]
    )
...
def student_performance(current_user, student_id):
    ...
    Teacher/Admin can view any student's performance.
    """
...
@role_required("teacher", "admin")
def course_analytics(current_user, course_id):
    """Teacher dashboard analytics for a course."""
    course = models.get_course_by_id(course_id)
    if not course:
        return err("Course not found", 404)
...
def admin_seed(current_user):
    """Admin: create demo data for testing."""
    _seed_demo_data()
...
def ai_chat(current_user):
    """
    AI chatbot for Q&A. Gemini-backed.
    Body: { message: str, context: str (optional topic/course context) }
    """
    if not gemini_model:
        return err("AI features are not configured. Set GEMINI_API_KEY.", 503)
    ...
    # Build a scoped educational prompt
    system_context = (
    ...
    try:
        ...
        log.info(f"AI chat: user={current_user['user_id']}, role={current_user['role']}")
...
def generate_content(current_user):
    """
    Teacher tool: Generate lesson summaries, quiz questions, or explanations.
    Body: { type: 'summary'|'quiz'|'explanation', topic: str, details: str }
    """
    if not gemini_model:
    ...
    prompts = {
        "summary": (
            ...
            f"Include key concepts, definitions, and 3 takeaways. Keep it under 300 words."
        ),
        "quiz": (
    ...
    try:
        response = gemini_model.generate_content(prompts[content_type])
        return ok({"content": response.text.strip(), "type": content_type, "topic": topic})
    except Exception as e:
...
def _seed_demo_data():
    """Create demo users, courses, and assignments for development."""

    # Create demo users
    users_data = [
        ("Admin User", "admin@demo.com", "admin123", "admin"),
        ("Dr. Priya Sharma", "teacher@demo.com", "teacher123", "teacher"),
        ("Rahil Khan", "student@demo.com", "student123", "student"),
        ("Ananya Singh", "ananya@demo.com", "student123", "student"),
    ]
    ...
    # Enroll students
    models.enroll_student(student1["id"], c1)
    models.enroll_student(student1["id"], c2)
    models.enroll_student(student2["id"], c1)
    ...
    a1 = models.create_assignment(
        ...
        rubric_keywords="supervised learning, labeled data, training, testing, classification, regression, prediction",
        max_score=100,
        due_date="2025-12-31"
    )
    ...
    a3 = models.create_assignment(
        ...
        description="Explain list comprehensions and when to use them over loops.",
        rubric_keywords="list comprehension, iteration, filter, expression, readable, concise, loop",
        max_score=50,
        due_date="2025-12-31"
    )

    # Create sample submissions and auto-grade them
    sample_answers = [
        (a1, student1["id"],
         "Supervised learning is a type of machine learning where the model is trained on labeled data. "
         "The training set contains input-output pairs. The model learns to predict outputs for new inputs. "
         "Examples include classification tasks like spam detection and regression tasks like house price prediction. "
         "After training, the model is evaluated on a testing set to measure its performance."),
        (a2, student1["id"],
         "A neural network consists of layers of neurons with weights connecting them. "
         "During training, backpropagation computes the gradient of the loss function with respect to weights. "
         "Gradient descent then updates the weights to minimise the loss. The activation function introduces non-linearity."),
        (a3, student1["id"],
